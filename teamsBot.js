...
teamsBot.message(/^\/delete(?:\s+(.*))?$/i, async (context, state) => {
  ...
  if (!args) {
    await context.sendActivity("❗ Please provide an employee ID or a name after `/delete`.\nExamples:\n- `/delete 12345`\n- `/delete John Doe`");
    return;
  }
  ...
  if (nameMatch) {
    ...
    // Try to also extract employee_id if present before/after name
    const idMatch = args.match(/(^|\s)(\d+)(\s|$)/);
    if (idMatch) {
      employeeId = idMatch[2].trim();
    }
  } else {
    // No "name" keyword, so check if it's all digits or all letters
    if (/^\d+$/.test(args)) {
      employeeId = args;
    } else {
      name = args;
  ...
  await context.sendActivity('🗑️ Deleting candidate...');
  ...
  try {
    ...
    if (typeof nifiResp === "object") {
      nifiResp = JSON.stringify(nifiResp, null, 2);
    }

  await context.sendActivity(
      `🗑️ Delete request sent for employee_id: \`${employeeId}\`, name: \`${name}\`.\nNiFi response:\n\`\`\`${nifiResp}\`\`\``
    );
  } catch (err) {
    let errMsg = err.response?.data?.nifi_response || err.message;
    if (typeof errMsg === "object") {
      errMsg = JSON.stringify(errMsg, null, 2);
    }
    await context.sendActivity(
      `❌ Failed to delete for employee_id \`${employeeId}\`, name: \`${name}\`:\n\`\`\`${errMsg}\`\`\``
    );
  }
});
...
teamsBot.message("/add", async (context, state) => {
  ...
  if (!employeeId) {
    await context.sendActivity("Usage: `/add <employee_id>`");
    return;
  }
...
teamsBot.activity(ActivityTypes.Message, async (context, state) => {
  ...
  if (state.conversation.awaitingLink) {
    state.conversation.resumeLink = text;
  ...
  if (state.conversation.awaitingConfirmation) {
    if (text.toLowerCase() === "yes") {
      ...
      try {
        ...
        const response = await axios.post("http://104.208.162.61:8083/add", payload, {
          headers: { "Content-Type": "application/json" },
        });
        ...
        await context.sendActivity(
          `✅ Resume link submitted! Server response:\n\n\`\`\`json\n${JSON.stringify(filteredData, null, 2)}\n\`\`\``
        );

      } catch (error) {
        await context.sendActivity(`❌ Failed to send data: ${error.message}`);
      }
    } else {
...
teamsBot.activity(ActivityTypes.Message, async (context, state) => {
  ...
  // If not in upload flow but file is attached, just echo back
  if (attachments && attachments.length > 0) {
    const attachment = attachments[0];
    // Instead of echoing file info, show friendly fallback message and command list
    await sendUnknownMessageWithCommands(context);
    return;
  }
 
  // If we reach here and there's text, it's a regular message (not a command)
  if (messageText) {
    // Instead of echoing back, show friendly fallback message and command list
    await sendUnknownMessageWithCommands(context);
    return;
  }
});
